...
@section('content')
    <div class="page__title">
        ...
        <img src="{{ asset('img/next.png') }}" alt="next">
        <a href="{{ route('admin.faculty', $faculty_id) }}">{{$faculty_name}}</a>
    </div>
    <div class="user-works block-no-padding">
        ...
        <div class="header">
            <div class="header-item">
                    <p>Кількість кафедр: {{$count_faculty}}</p>
                </div>
                        ...
                        <label for="sort">
                            ...
                            @if (!empty($get_req))
                                <select name="value" id="sort">
                                    ...
                                    >назва кафедри</option>
                                    <option value="2"
                                    {{$get_req['val'] == 2 ? 'selected' : null}}
                                    >завідувач кафедри</option>
                                </select>
                            @else
                            <select name="value" id="sort">
                                <option value="1">назва кафедри</option>
                                <option value="2">завідувач кафедри</option>
                            </select>
                            @endif
                        </label>
                </div>
            ...
            <table class="main-table" style="margin-top:20px; width:100%">
                ...
                @foreach ($departaments as $departament)
                    <tr>
                        ...
                        <td style="padding: 0; text-align:left;">
                           ...
                           <div data-req-id="{{ $departament->id }}" class="show-question">
                            <img src="{{ asset('img/detail.png') }}" alt="add">
                        </div>
                        <div class="modal modal-quest" data-req-id="{{ $departament->id }}">
                            <div class="modal-header modal-header-req" data-req-id="{{ $departament->id }}">
                                <img  src="{{ asset('img/delete.png') }}" alt="close">
                            </div>
                            <div class="modal-body">
                                <form action="{{ route('admin.departamentEdit') }}" method="POST">
                                    @csrf
                                    <input required type="text" name="id" hidden value="{{ $departament->id }}">
                                    <label for="name">
                                        <p>Назва кафедри</p>
                                        <input required type="text" name="departament_name" class="text-input" value="{{ $departament->departament_name }}">
                                    </label>
                                    <label for="head">
                                        <p>Завідувач кафедри</p>
                                        <input type="text" name="head_departament" class="text-input" value="{{ $departament->head_departament }}">
                                    </label>
                                    <p>Факультет</p>
                                    <select id="faculty_id" name="faculty_id" class="text-input" style="width: 100%;">
                                        @foreach ($facultes as $faculty)
                                            <option value="{{ $faculty->id }}"
                                                {{ $faculty->id == $departament->faculty_id ? 'selected' : null }}
                        ...
                        <td>
                            <div data-req-id="{{ $departament->id }}" class="show-bar">
                ...
                <div class="modal" id="modal-answer">
                    ...
                    <div class="id"></div>
                    <form action="{{ route('admin.deleteDepartament') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        ...
                        >Ви дійсно хочете видалити кафедру?
                            При видаленні кафедри знищуватися всі співробітники цієї кафедри і їх роботи</p>
            ...
            <div class="wrapped-elements">
                {{ $departaments->appends(request()->query())->links() }}
            </div>
            @endif
        @endif
    </div>
    ...
    <div class="modal new-dep-model" style="width: 500px;">
        ...
        <div class="modal-body">
            <form action="{{ route('admin.departamentNew') }}" method="POST">
                @csrf
                <label for="name">
                    <p>Назва кафедри</p>
                    <input style="width:100%" required type="text" name="departament_name" class="text-input">
                </label>
                <label for="head">
                    <p>Завідувач кафедри</p>
                    <input style="width:100%" type="text" name="head_departament" class="text-input">
                </label>
                <p>Факультет</p>
                <select id="faculty_id" name="faculty_id" class="text-input" style="width: 100%;">
                    @foreach ($facultes as $faculty)
                        <option value="{{ $faculty->id }}"
                        {{ $faculty_name == $faculty->faculty_name ? 'selected' : null }}
                            >{{ $faculty->faculty_name }}</option>
                    @endforeach
                </select>
                <div class="f-btn">

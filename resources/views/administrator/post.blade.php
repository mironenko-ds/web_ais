...
@section('content')
    <div class="page__title">
        ...
        <a href="{{ route('admin.postShow') }}">Посади</a>
    </div>
    <div class="user-works block-no-padding">
        <div class="header">
            <div class="header-item">
                    <p>Кількість посад: {{$posts->total()}}</p>
                </div>
                @if ($posts->total() != 0)
                <div class="header-item">
                    <form id="go-sort" action="" method="get">
                        <label for="sort">
                            ...
                            @else
                            <select name="value" id="sort">
                                <option value="1">назва посади</option>
                            </select>
                            @endif
                        </label>
                </div>
            ...
            <table class="main-table" style="margin-top:20px; width:100%">
                ...
                @foreach ($posts as $post)
                    <tr>
                        ...
                        <td style="padding: 0; text-align:left;">
                           ...
                           <div data-req-id="{{ $post->id }}" class="show-question">
                            <img src="{{ asset('img/detail.png') }}" alt="add">
                        </div>
                        <div class="modal modal-quest" data-req-id="{{ $post->id }}">
                            <div class="modal-header modal-header-req" data-req-id="{{ $post->id }}">
                                <img  src="{{ asset('img/delete.png') }}" alt="close">
                            </div>
                            <div class="modal-body">
                                <form action="{{ route('admin.postEdit') }}" method="POST">
                                    @csrf
                                    <input required type="text" name="id" hidden value="{{ $post->id }}">
                                    <label for="name">
                                        <p>Назва посади</p>
                                        <input required type="text" name="post_name" class="text-input" value="{{ $post->post_name }}">
                                    </label>
                                    <div class="f-btn">
                        ...
                        <td>
                            <div data-req-id="{{ $post->id }}" class="show-bar">
                ...
                <div class="modal" id="modal-answer">
                    ...
                    <div class="id"></div>
                    <form action="{{ route('admin.postDelete') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <input type="text" hidden class="feedback_id" name="post_id">
                        <p style="padding: 5px;font-size: 18px;font-weight: 500; font-family:Montserrat;text-align:center;"
                        >Ви дійсно хочете видалити посаду?</p>
    ...
    <div class="modal new-dep-model" style="width: 500px;">
        ...
        <div class="modal-body">
            <form action="{{ route('admin.postNew') }}" method="POST">
                ...
                <label for="name">
                    <p>Назва посади</p>
                    <input style="width:100%" required type="text" name="post_name" class="text-input">
                </label>
                <div class="f-btn">
